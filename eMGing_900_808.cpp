...
eMGing_900_808::eMGing_900_808(int8_t rst)
{
  apn = F("eMGing_net");
  apnusername = 0;
  apnpassword = 0;
  mySerial = 0;
  httpsredirect = false;
  useragent = F("Shield_eMGing_900/808");
  ok_reply = F("OK");
}

uint8_t eMGing_900_808::type(void) {
  return _type;
}

boolean eMGing_900_808::begin(Stream &port) {
  mySerial = &port;
 ...
 int16_t timeout = 7000;
  ...
  if (prog_char_strstr(replybuffer, (prog_char *)F("SIM808 R14")) != 0) {
    ...
    _type = EMGING_808_V1;
  } else if (prog_char_strstr(replybuffer, (prog_char *)F("SIM800 R11")) != 0) {
    _type = EMGING_900;
  ...
  sendCheckReply(F("AT+CPMS=\"" PREF_SMS_STORAGE "\""), ok_reply);
#endif
...
/********* Serial port ********************************************/
boolean eMGing_900_808::setBaudrate(uint16_t baud) {
  return sendCheckReply(F("AT+IPREX="), baud, ok_reply);
}

/********* Real Time Clock ********************************************/

boolean eMGing_900_808::readRTC(uint8_t *year, uint8_t *month, uint8_t *date, uint8_t *hr, uint8_t *min, uint8_t *sec) {
  uint16_t v;
...
boolean eMGing_900_808::enableRTC(uint8_t i) {
...
boolean eMGing_900_808::getBattVoltage(uint16_t *v) {
  return sendParseReply(F("AT+CBC"), F("+CBC: "), v, ',', 2);
}

/* returns value in mV (uint16_t) */
boolean eMGing_3G::getBattVoltage(uint16_t *v) {
  float f;
...
boolean eMGing_900_808::getBattPercent(uint16_t *p) {
  return sendParseReply(F("AT+CBC"), F("+CBC: "), p, ',', 1);
}

boolean eMGing_900_808::getADCVoltage(uint16_t *v) {
  return sendParseReply(F("AT+CADC?"), F("+CADC: 1,"), v);
}

/********* SIM ***********************************************************/

uint8_t eMGing_900_808::unlockSIM(char *pin)
{
...
uint8_t eMGing_900_808::getSIMCCID(char *ccid) {
  getReply(F("AT+CCID"));
...
uint8_t eMGing_900_808::getIMEI(char *imei) {
  getReply(F("AT+GSN"));
...
uint8_t eMGing_900_808::getNetworkStatus(void) {
  uint16_t status;
...
uint8_t eMGing_900_808::getRSSI(void) {
  uint16_t reply;
...
boolean eMGing_900_808::setAudio(uint8_t a) {
...
uint8_t eMGing_900_808::getVolume(void) {
  uint16_t reply;
...
boolean eMGing_900_808::setVolume(uint8_t i) {
  return sendCheckReply(F("AT+CLVL="), i, ok_reply);
}


boolean eMGing_900_808::playDTMF(char dtmf) {
  char str[4];
  str[0] = '\"';
  str[1] = dtmf;
  str[2] = '\"';
...
boolean eMGing_900_808::playToolkitTone(uint8_t t, uint16_t len) {
  return sendCheckReply(F("AT+STTONE=1,"), t, len, ok_reply);
}

boolean eMGing_3G::playToolkitTone(uint8_t t, uint16_t len) {
...
boolean eMGing_900_808::setMicVolume(uint8_t a, uint8_t level) {
  // 0 is headset, 1 is external audio
  if (a > 1) return false;
...
boolean eMGing_900_808::FMradio(boolean onoff, uint8_t a) {
  if (! onoff) {
...
boolean eMGing_900_808::tuneFMradio(uint16_t station) {
...
boolean eMGing_900_808::setFMVolume(uint8_t i) {
...
int8_t eMGing_900_808::getFMVolume() {
  uint16_t level;
...
int8_t eMGing_900_808::getFMSignalLevel(uint16_t station) {
  // Fail if FM station is outside allowed range.
  ...
  getReply(F("AT+FMSIGNAL="), station, DEFAULT_TIMEOUT_MS);
...
boolean eMGing_900_808::setPWM(uint16_t period, uint8_t duty) {
...
/********* CALL PHONES **************************************************/
boolean eMGing_900_808::callPhone(char *number) {
...
uint8_t eMGing_900_808::getCallStatus(void) {
...
boolean eMGing_900_808::hangUp(void) {
  return sendCheckReply(F("ATH0"), ok_reply);
}

boolean eMGing_3G::hangUp(void) {
  getReply(F("ATH"));

  return (prog_char_strstr(replybuffer, (prog_char *)F("VOICE CALL: END")) != 0);
}

boolean eMGing_900_808::pickUp(void) {
  return sendCheckReply(F("ATA"), ok_reply);
}

boolean eMGing_3G::pickUp(void) {
  return sendCheckReply(F("ATA"), F("VOICE CALL: BEGIN"));
}


void eMGing_900_808::onIncomingCall() {

  DEBUG_PRINT(F("> ")); DEBUG_PRINTLN(F("Incoming call..."));

  eMGing_900_808::_incomingCall = true;
}

boolean eMGing_900_808::_incomingCall = false;

boolean eMGing_900_808::callerIdNotification(boolean enable, uint8_t interrupt) {
  if(enable){
...
boolean eMGing_900_808::incomingCallNumber(char* phonenum) {
  //+CLIP: "<incoming phone number>",145,"",0,"",0
  if(!eMGing_900_808::_incomingCall)
    return false;
  ...
  eMGing_900_808::_incomingCall = false;
  return true;
}

/********* SMS **********************************************************/

uint8_t eMGing_900_808::getSMSInterrupt(void) {
  uint16_t reply;
...
boolean eMGing_900_808::setSMSInterrupt(uint8_t i) {
  return sendCheckReply(F("AT+CFGRI="), i, ok_reply);
}

int8_t eMGing_900_808::getNumSMS(void) {
  uint16_t numsms;
...
// printouts!
boolean eMGing_900_808::readSMS(uint8_t i, char *smsbuff,
...
// otherwise false.
boolean eMGing_900_808::getSMSSender(uint8_t i, char *sender, int senderlen) {
...
boolean eMGing_900_808::deleteSMS(uint8_t i) {
    if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) return -1;
  // read an sms
...
boolean eMGing_900_808::sendUSSD(char *ussdmsg, char *ussdbuff, uint16_t maxlen, uint16_t *readlen) {
...
boolean eMGing_900_808::enableNetworkTimeSync(boolean onoff) {
  if (onoff) {
...
boolean eMGing_900_808::enableNTPTimeSync(boolean onoff, FlashStringPtr ntpserver) {
  if (onoff) {
...
boolean eMGing_900_808::getTime(char *buff, uint16_t maxlen) {
...
boolean eMGing_900_808::enableGPS(boolean onoff) {
  uint16_t state;

  // first check if its already on or off

  if (_type == EMGING_808_V2) {
...
boolean eMGing_3G::enableGPS(boolean onoff) {
  uint16_t state;

  // first check if its already on or off
  if (! eMGing_900_808::sendParseReply(F("AT+CGPS?"), F("+CGPS: "), &state) )
    return false;
...
int8_t eMGing_900_808::GPSstatus(void) {
  if (_type == EMGING_808_V2) {
  ...
  if (_type == EMGING_3G_A || _type == EMGING_3G_E) {
    // EMGING_3G doesn't have an explicit 2D/3D fix status.
    // Instead just look for a fix and if found assume it's a 3D fix.
...
uint8_t eMGing_900_808::getGPS(uint8_t arg, char *buffer, uint8_t maxbuff) {
  int32_t x = arg;

  if ( (_type == EMGING_3G_A) || (_type == EMGING_3G_E) ) {
    getReply(F("AT+CGPSINFO"));
  } else if (_type == EMGING_808_V1) {
...
boolean eMGing_900_808::getGPS(float *lat, float *lon, float *speed_kph, float *heading, float *altitude) {
...
boolean eMGing_900_808::enableGPSNMEA(uint8_t i) {
...
boolean eMGing_900_808::enableGPRS(boolean onoff) {
...
boolean eMGing_3G::enableGPRS(boolean onoff) {
...
uint8_t eMGing_900_808::GPRSstate(void) {
  uint16_t state;
...
void eMGing_900_808::setGPRSNetworkSettings(FlashStringPtr apn,
              FlashStringPtr username, FlashStringPtr password) {
  this->apn = apn;
  this->apnusername = username;
  this->apnpassword = password;
}

boolean eMGing_900_808::getGSMLoc(uint16_t *errorcode, char *buff, uint16_t maxlen) {
...
boolean eMGing_900_808::getGSMLoc(float *lat, float *lon) {
...
boolean eMGing_900_808::TCPconnect(char *server, uint16_t port) {
  flushInput();
...
boolean eMGing_900_808::TCPclose(void) {
  return sendCheckReply(F("AT+CIPCLOSE"), ok_reply);
}

boolean eMGing_900_808::TCPconnected(void) {
...
boolean eMGing_900_808::TCPsend(char *packet, uint8_t len) {
...
uint16_t eMGing_900_808::TCPavailable(void) {
  uint16_t avail;
...
uint16_t eMGing_900_808::TCPread(uint8_t *buff, uint8_t len) {
  uint16_t avail;
...
boolean eMGing_900_808::HTTP_init() {
  return sendCheckReply(F("AT+HTTPINIT"), ok_reply);
}

boolean eMGing_900_808::HTTP_term() {
  return sendCheckReply(F("AT+HTTPTERM"), ok_reply);
}

void eMGing_900_808::HTTP_para_start(FlashStringPtr parameter,
                                    boolean quoted) {
  flushInput();
...
boolean eMGing_900_808::HTTP_para_end(boolean quoted) {
  if (quoted)
    mySerial->println('"');
  else
    mySerial->println();
...
boolean eMGing_900_808::HTTP_para(FlashStringPtr parameter,
                                 const char *value) {
...
boolean eMGing_900_808::HTTP_data(uint32_t size, uint32_t maxTime) {
  flushInput();
...
boolean eMGing_900_808::HTTP_action(uint8_t method, uint16_t *status,
...
boolean eMGing_900_808::HTTP_readall(uint16_t *datalen) {
...
boolean eMGing_900_808::HTTP_ssl(boolean onoff) {
  return sendCheckReply(F("AT+HTTPSSL="), onoff ? 1 : 0, ok_reply);
}

/********* HTTP HIGH LEVEL FUNCTIONS ***************************/

boolean eMGing_900_808::HTTP_GET_start(char *url,
...
/*
boolean eMGing_3G::HTTP_GET_start(char *ipaddr, char *path, uint16_t port
				      uint16_t *status, uint16_t *datalen){
  ...
  // HTTP GET
  if (! HTTP_action(HTTP_GET, status, datalen))
    return false;
...
void eMGing_900_808::HTTP_GET_end(void) {
  HTTP_term();
}

boolean eMGing_900_808::HTTP_POST_start(char *url,
              FlashStringPtr contenttype,
...
void eMGing_900_808::HTTP_POST_end(void) {
  HTTP_term();
}

void eMGing_900_808::setUserAgent(FlashStringPtr useragent) {
  this->useragent = useragent;
}

void eMGing_900_808::setHTTPSRedirect(boolean onoff) {
  httpsredirect = onoff;
}

/********* HTTP HELPERS ****************************************/

boolean eMGing_900_808::HTTP_setup(char *url) {
  // Handle any pending
  HTTP_term();
...
boolean eMGing_900_808::expectReply(FlashStringPtr reply,
                                   uint16_t timeout) {
  readline(timeout);
...
inline int eMGing_900_808::available(void) {
  return mySerial->available();
}

inline size_t eMGing_900_808::write(uint8_t x) {
  return mySerial->write(x);
}

inline int eMGing_900_808::read(void) {
  return mySerial->read();
}

inline int eMGing_900_808::peek(void) {
  return mySerial->peek();
}

inline void eMGing_900_808::flush() {
  mySerial->flush();
}

void eMGing_900_808::flushInput() {
...
uint16_t eMGing_900_808::readRaw(uint16_t b) {
  uint16_t idx = 0;
...
uint8_t eMGing_900_808::readline(uint16_t timeout, boolean multiline) {
  uint16_t replyidx = 0;
...
uint8_t eMGing_900_808::getReply(char *send, uint16_t timeout) {
  flushInput();
...
uint8_t eMGing_900_808::getReply(FlashStringPtr send, uint16_t timeout) {
  flushInput();
...
uint8_t eMGing_900_808::getReply(FlashStringPtr prefix, char *suffix, uint16_t timeout) {
  flushInput();
...
// Send prefix, suffix, and newline. Return response (and also set replybuffer with response).
uint8_t eMGing_900_808::getReply(FlashStringPtr prefix, int32_t suffix, uint16_t timeout) {
  flushInput();
...
uint8_t eMGing_900_808::getReply(FlashStringPtr prefix, int32_t suffix1, int32_t suffix2, uint16_t timeout) {
  flushInput();
...
// Send prefix, ", suffix, ", and newline. Return response (and also set replybuffer with response).
uint8_t eMGing_900_808::getReplyQuoted(FlashStringPtr prefix, FlashStringPtr suffix, uint16_t timeout) {
  flushInput();
...
boolean eMGing_900_808::sendCheckReply(char *send, char *reply, uint16_t timeout) {
...
boolean eMGing_900_808::sendCheckReply(FlashStringPtr send, FlashStringPtr reply, uint16_t timeout) {
	if (! getReply(send, timeout) )
		return false;
...
boolean eMGing_900_808::sendCheckReply(char* send, FlashStringPtr reply, uint16_t timeout) {
  if (! getReply(send, timeout) )
	  return false;
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}
...
boolean eMGing_900_808::sendCheckReply(FlashStringPtr prefix, char *suffix, FlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify response matches reply parameter.
boolean eMGing_900_808::sendCheckReply(FlashStringPtr prefix, int32_t suffix, FlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, suffix2, and newline.  Verify response matches reply parameter.
boolean eMGing_900_808::sendCheckReply(FlashStringPtr prefix, int32_t suffix1, int32_t suffix2, FlashStringPtr reply, uint16_t timeout) {
  getReply(prefix, suffix1, suffix2, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, ", suffix, ", and newline.  Verify response matches reply parameter.
boolean eMGing_900_808::sendCheckReplyQuoted(FlashStringPtr prefix, FlashStringPtr suffix, FlashStringPtr reply, uint16_t timeout) {
  getReplyQuoted(prefix, suffix, timeout);
  return (prog_char_strcmp(replybuffer, (prog_char*)reply) == 0);
}


boolean eMGing_900_808::parseReply(FlashStringPtr toreply,
...
// response.
boolean eMGing_900_808::parseReplyQuoted(FlashStringPtr toreply,
...
boolean eMGing_900_808::sendParseReply(FlashStringPtr tosend,
...
boolean eMGing_3G::sendParseReply(FlashStringPtr tosend,
				      FlashStringPtr toreply,
...
boolean eMGing_3G::parseReply(FlashStringPtr toreply,
          float *f, char divider, uint8_t index) {
  char *p = prog_char_strstr(replybuffer, (prog_char*)toreply);  // get the pointer to the voltage
  if (p == 0) return false;
  p+=prog_char_strlen((prog_char*)toreply);
